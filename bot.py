...
class GistStorage:
    def __init__(self, gist_id: str, token: str, filename: str = "thirdwave_seen_messages.json",
        ...
        self.enabled = bool(token)           # Only token is required; gist_id is auto-created if missing
            ...
            async with httpx.AsyncClient(timeout=15.0) as http:
                res = await http.post(
                    ...
                    json={
                        ...
                        "files": {
                            self.filename: {
                                "content": json.dumps({"seen": {}, "bot": "THIRDWAVE_OTP_WAVE", "count": 0}, indent=2)
                            }
                        }
                    }
                )
...
async def send_startup_announcement(application: Application):
    ...
    status_header = "🔄 <b>OTP WAVE AUTO-REFRESH COMPLETED</b>" if is_restart else "🚀 <b>OTP WAVE ONLINE (Admin Alert)</b>"
    
    admin_msg = (
        f"{status_header}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"• <b>Status:</b> <code>Active & Monitoring Live OTPs ✅</code>\n"
        f"• <b>Platform:</b> <code>Thirdwave IPRN ({THIRDWAVE_BASE_URL})</code>\n"
...
async def main():
    ...
    try:
        ...
        # Explicitly dispatch startup announcement right after initialization
